...
export default function TaskModal({ initial, createdBy, onSave, onClose }: Props) {
  const [title, setTitle] = useState(initial?.title ?? '')
  const [owner, setOwner] = useState(initial?.owner ?? createdBy)
      ...
      <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-5 space-y-4">
          <label className="block">
            <span className="text-xs font-medium text-neutral-500">업무명</span>
            <input
              type="text"
              value={title}
          ...
          <label className="block">
            <span className="text-xs font-medium text-neutral-500">담당자</span>
            <input
              type="text"
              value={owner}
          ...
          <label className="block">
            <span className="text-xs font-medium text-neutral-500">주기</span>
            <select
              value={frequency}
          ...
          <label className="block">
            <span className="text-xs font-medium text-neutral-500">마감일</span>
            <input
              type="date"
              value={due}
