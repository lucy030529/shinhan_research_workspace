...
import { MOCK_COVERAGE, MOCK_GAP_RATIO, MOCK_TASKS } from '../data/mock'
...
export default function DashboardPage() {
  ...
  const dueSoon = [...MOCK_COVERAGE]
    .map((c) => ({ ...c, days: daysUntil(c.nextDue) }))
    .filter((c) => c.days <= 45)
    .sort((a, b) => a.days - b.days)

  const warnings = MOCK_GAP_RATIO.filter((g) => Math.abs(g.gapRatio) >= GAP_WARNING_THRESHOLD)
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatTile label="커버리지 종목" value={MOCK_COVERAGE.length} hint="담당 종목 수" tone="brand" />
        <StatTile
          label="45일 내 업데이트 필요"
